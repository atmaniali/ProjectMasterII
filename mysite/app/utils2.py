...
from app.models import Promethee_file
import numpy as np
import pandas as pd

def numpy_to_csv(matrix, names):
    ...
    fil = df.to_csv(urls)
    f =  Promethee_file.objects.create(name = names, files = fil) 
    return f
import csv    
def weight_to_csv(weights, names):
    ...
    with open(urls, 'w') as f:
       write = csv.writer(f)
       w = write.writerow(weights)
    return w
    #    f =  Promethee_file.objects.create(name = names, files = weights) 
ma  = [[1, 2], [3, 4],[5,6]]
na = "test"
a = numpy_to_csv(ma, na)
print(a)
